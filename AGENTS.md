...
- Use **Atom Studio** for user-visible names, window titles, exports, and packaged downloads. Keep the existing Qt application/organization storage identifiers in `src/main.cpp` stable so settings and managed Python environments remain available; use `applicationDisplayName()` for display text.
...
- `StructureModel::atomRadiusType` selects covalent (default) or Alvarez van der Waals display radii globally. Apply changes lazily to inactive documents, preserve relative per-atom scales, and style new/reset/replicated/Python-created atoms consistently. Radius changes invalidate viewport geometry and picking/BVH data, not covalent bond detection. Alvarez-missing elements retain the documented legacy van der Waals fallbacks in `ElementData.cpp`.
- The Atoms sidebar's `PerAtomProperties` panel uses a virtualized `AtomPropertiesModel`, ordered by stable zero-based import atom IDs (survivors keep their IDs; new/replicated atoms follow). Show its plain title and bounded scrollable list whenever Atoms is expanded, without a separate dropdown. Size the ID and Species columns to their contents and keep column gaps compact. Species color/radius edits affect only the active document, including matching replicas and newly inserted Python atoms. Radii are in Å before the viewport's global Atom Scale multiplier. A radius-scheme change clears explicit species radii, including inactive documents that skip an entire scheme round trip; species colors persist. Reset clears species overrides, replication preserves them, and raw input remains immutable. Reuse the shared color popup, close it on scrolling/document replacement, and discard stale radius drafts on document replacement. Capture and Python edit locks disable these editors.
...
- Atom hover text uses `ID | Element | Position` in both viewports. Resolve IDs from the active document's stable zero-based display IDs, matching the per-atom table after deletion or Python reordering.
...
- Regular atom/bond strokes use angstrom shell widths (default 0.05 Å, sidebar range 0.01–0.50 Å), independent of zoom, device-pixel ratio, and Atom Scale. The Selection sidebar edits persistent per-object stroke color/width on selected atoms and bonds; it never edits selection highlighting. Selected objects always show a red 0.15 Å highlight instead of their saved stroke, even if the saved stroke is thicker or Show Strokes is off. Deselecting reveals the saved stroke. Show Strokes controls all persistent strokes. An explicit structure-wide color or width edit clears that property's object overrides in every loaded current document (including inactive structures), so the shared setting applies to every object. Preserve the other stroke property, selections, and raw originals. Reapplying the same global value also clears the matching overrides; merely opening a section or switching documents does not. Overrides survive selection changes, document switches, cloning/deletion, surviving bond detection and Python updates by stable identity; Reset selected objects clears them. Replication/reset from raw retain the existing geometry-reset semantics. Capture/Python edit locks disable selected-object controls. Metal raster and ray tracing upload stroke appearance and update conservative outline bounds without rebuilding geometry. OpenGL stroke rendering remains unimplemented.
